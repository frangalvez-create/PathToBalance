...
struct ContentView: View {
                        ...
                        ScrollViewReader { proxy in
                            ScrollView {
                                VStack(alignment: .leading, spacing: 8) {
                                    ...
                                    // AI Response (Jedi Philosophy: and Balanced Mindset: bolded as paragraph titles)
                        ...
                        VStack {
                            ...
                            HStack {
                                ...
                                Button(action: {
                                    ...
                                    if isGeneratingAI {
                                        ProgressView()
                                            .progressViewStyle(CircularProgressViewStyle(tint: Color(hex: "39765A")))
                            ...
                            ScrollViewReader { proxy in
                                ScrollView {
                                    VStack(alignment: .leading, spacing: 8) {
                                        ...
                                        // AI Response (Jedi Philosophy: and Balanced Mindset: bolded as paragraph titles for guided/open)
    ...
    private var authenticationView: some View {
        VStack(spacing: 20) {
            ...
            // Welcome to text - 22pt regular, Accent #39765A
            Text("Welcome to")
                .font(.system(size: 22))
                .foregroundColor(Color(hex: "39765A"))
                    ...
                    VStack(spacing: 15) {
                        ...
                        Button(action: {
                            ...
                            Text("Back")
                                ...
                                .overlay(
                                    RoundedRectangle(cornerRadius: 8)
                                        .stroke(Color(hex: "39765A"), lineWidth: 1)
                                )
                        }
...
Paragraph 1: empathetically acknowledge the user focus/concern and provide a factual and mindset explanation relevant to the input.
Paragraph 2: MUST begin with "Jedi Philosophy: " (include the colon and space) then reference themes commonly depicted in Star Wars storytelling and teach how Jedi philosophy is shown interpreting a narrative familiar but not directly to the question asked.
...
"""
        ...
        let goalText = goal.isEmpty ? "balanced" : goal
...
Trainer: {fuq_ai_response}
User: {content}
Output Requirements:
Produce a concise, information-dense summary in two short paragraphs.
Paragraph 1: Provide a succinct summary of the above trainer/user conversation.
Paragraph 2: provide one positive achievable action aligned with calm leadership, patience, and self-mastery
...
Reference constraints
Max: 150 words.
Capabilities: Use your trained knowledge and interpretation of Jedi philosophy in the Star Wars universe, drawing from themes commonly depicted in the films and series
Important: Keep reasoning minimal and respond directly.
"""
            ...
            if analyzerViewModel.moodCounts.isEmpty {
                VStack(spacing: 12) {
                    Image(systemName: "chart.bar.doc.horizontal")
                        .font(.system(size: 28))
                        .foregroundColor(Color(hex: "39765A").opacity(0.4))
        ...
        VStack(spacing: 6) {
            ...
            GeometryReader { geo in
                ...
                ZStack {
                    ...
                    // Balance Score - Top Right (y = 0, ends at y = 156pt)
                    VStack(spacing: 25) {
                        Text("Balance Score")
                            .font(.system(size: 16))
                            .foregroundColor(Color(hex: "545555"))
            ...
            .frame(height: 156) // Balance Score height (tallest card)
        }
        .frame(maxWidth: .infinity, alignment: .leading)
    }
    ...
    private var summarySection: some View {
        VStack(alignment: .leading, spacing: 6) {
            ...
            if let summaryText = analyzerViewModel.summaryText, !summaryText.isEmpty {
                formattedSummaryView(summaryText, textColor: "39765A")
                    .multilineTextAlignment(.leading)
                    .lineLimit(nil)
                    .fixedSize(horizontal: false, vertical: true)
                    .padding(16)
    ...
    /// Formats journal AI response with "Jedi Philosophy:" and "Balanced Mindset:" as bold paragraph titles (like Summary/Next Week's Goal in analyzer)
    @ViewBuilder
    private func formattedJournalAIResponseView(_ text: String, textColor: String = "39765A") -> some View {
        // Backward compatibility: old entries may use "Jedi Interpretation:" instead of "Jedi Philosophy:"
        let normalizedText = text.replacingOccurrences(of: "\u{2019}", with: "'")
            .replacingOccurrences(of: "Jedi Interpretation:", with: "Jedi Philosophy:")
        ...
        if let jediRange = normalizedText.range(of: jediMarker),
           let actionRange = normalizedText.range(of: actionMarker), actionRange.lowerBound > jediRange.lowerBound {
            // Both markers present: Paragraph 1 | Jedi Philosophy: P2 | Balanced Mindset: P3
    ...
    var infoPopupView: some View {
        ZStack {
            ...
            VStack(alignment: .leading, spacing: 8) {
                ...
                Text("• Periodically, explore questions will ask about previous entries.")
                    .font(.system(size: 15))
                    .foregroundColor(Color(hex: "545555"))
                ...
                Text("Whether you like to train early in the morning or right before bed, you can set your own reminder times in the Notifications section of the User Settings page.")
                    .font(.system(size: 15))
                    .foregroundColor(Color(hex: "545555"))
                ...
                Text("If you add your information (gender, occupation and birthdate) in the User Settings page (Here), the insight will include this in its analysis.")
                    .font(.system(size: 15))
                    .foregroundColor(Color(hex: "545555"))
                    .onTapGesture {
                        showInfoPopup = false
    ...
    var q3InfoPopupView: some View {
        ZStack {
            ...
            VStack(alignment: .leading, spacing: 8) {
                ...
                Text("• The training question changes daily. The topics include gratitude, mindset, mental and physical health and other similar topics.")
                    .font(.system(size: 15))
                    .foregroundColor(Color(hex: "545555"))
                
                Text("• These questions are designed to challenge your mind and reflect on specific aspects of your life and well-being. A balanced mind in any situation is the goal.")
                    .font(.system(size: 15))
                    .foregroundColor(Color(hex: "545555"))
                
                Text("• After completing your entry, you have an option to tap the \"Insight\" button to receive Jedi philosophy interpretation and commentary.")
                    .font(.system(size: 15))
                    .foregroundColor(Color(hex: "545555"))
    ...
    var goalInfoPopupView: some View {
        ZStack {
            ...
            VStack(spacing: 8) {
                ...
                Text("You can enter a personal goal in this field (e.g. 'more balanced, less dark'), and the insights will be tailored around it.")
                    .font(.system(size: 15))
                    .foregroundColor(Color(hex: "545555"))
                    .multilineTextAlignment(.leading)
                    .frame(maxWidth: .infinity, alignment: .leading)
    ...
    var q4InfoPopupView: some View {
        ZStack {
            ...
            VStack(alignment: .leading, spacing: 8) {
                ...
                Text("• Calculates training entry statistics")
                    .font(.system(size: 15))
                    .foregroundColor(.white)
                    .multilineTextAlignment(.leading)
                    .frame(maxWidth: .infinity, alignment: .leading)
                
                Text("• Generates a Balanced Score - how balanced you are based on your entries (0-100)")
                    .font(.system(size: 15))
                    .foregroundColor(.white)
                    .multilineTextAlignment(.leading)
                    .frame(maxWidth: .infinity, alignment: .leading)
                ...
                Text("• Weekly Analysis: 2 days of training entries / week")
                    .font(.system(size: 15))
                    .foregroundColor(.white)
                    .multilineTextAlignment(.leading)
                    .frame(maxWidth: .infinity, alignment: .leading)
                
                Text("• Monthly Analysis: 9 days of training entries / month")
                    .font(.system(size: 15))
                    .foregroundColor(.white)
                    .multilineTextAlignment(.leading)
                    .frame(maxWidth: .infinity, alignment: .leading)

...
class SupabaseService: ObservableObject {
    ...
    func generateWeeklyAnalyzerPrompt(content: String) -> String {
        let promptTemplate = """
Analyze how Jedi philosophy in the Star Wars universe would interpret the following journal entries:
"{content}"
Output format (exactly):
1st paragraph:
mood1(#), mood2(#), mood3(#)
2nd paragraph (two bullets):
- Summary: <summary of user's latest path to balance>
- Next Week's Goal: <calm leadership, patience, and self-mastery action steps + weekly balanced goal>
3rd paragraph:
<balance score only, number from 60–100>
Important:
- Moods must be one word each.
- Score must be the final line and only the number—no text.
- Focus on mental growth, and balanced living.
"""
